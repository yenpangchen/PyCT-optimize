        ...
        if self.dump: # Logging output section
            if self._omit(_id):
                ...
                with open(f'{_id}.csv', 'w') as f:
                    ...
                    # mkdir paper_statistics && echo "ID|Function|Line Coverage|Time (sec.)|# of SMT files|# of SAT|Time of SAT|# of UNSAT|Time of UNSAT|# of OTHERWISE|Time of OTHERWISE" > output.csv2 && dump=True pytest integration_test_pyconbyte.py --workers 4 && cp /dev/null paper_statistics/pyconbyte_run_pyconbyte.csv && cat *.csv >> output.csv2 && rm -f *.csv && mv output.csv2 paper_statistics/pyconbyte_run_pyconbyte.csv
