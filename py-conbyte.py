...
parser.add_argument("input", metavar="initial_vector", help="list of initial arguments to be passed into the target function\nPlease note that the double quotes outside the list cannot be omitted!\nEx1: func(a=1,b=2) -> \"[1,2]\"\nEx2: func(a='',b='') -> \"['','']\"")
...
parser.add_argument("--ignore_return", dest="ignore_return", action='store_true', help="disable examination of return values if they are not picklable.")
parser.add_argument("--include_exception", dest="include_exception", action='store_true', help="also update coverage statistics when the target function throws an exception.")
...
print("\nTotal iterations:", engine.explore(args.modpath, eval(args.input), root=args.root, funcname=args.func,
                                            max_iterations=args.iter, timeout=args.timeout2, check_return=not args.ignore_return,
                                            include_exception=args.include_exception, deadcode=None, lib=args.lib))
##############################################################################
...
print('LIST_OF_ALL_INPUTS:' + str(engine.inputs + engine.errors))