...
def _wrap_import(name, globals=None, locals=None, fromlist=(), level=0):
    ...
    #######################################################################
    # if module.__name__ in ['make_server', 'pydoc']: # in XSS testing
    if module.__name__ not in ['conbyte.concolic_types.concolic_int',
        ...
        try:
            ...
            if not (isinstance(exception, OSError) and message in ['could not get source code',
                                                                   'source code not available']) \
            and not (isinstance(exception, TypeError) and message.endswith('is a built-in module')):
                traceback.print_exc()
