...
class PathToConstraint:
    ...
    def which_branch(self, conbool):
        ...
        if c is None and cneg is None:
            c = self.current_constraint.add_child(p, conbyte.global_utils.engine.extend_vars, conbyte.global_utils.engine.extend_queries)
            c.processed = True # 只是給我們看的，程式流程用不到這個
            cneg = self.current_constraint.add_child(pneg, conbyte.global_utils.engine.extend_vars, conbyte.global_utils.engine.extend_queries)
